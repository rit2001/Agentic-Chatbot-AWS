...
# --- NEW IMPORTS FOR GROQ AND HUGGINGFACE ---
...
# LLM - Switched to Groq (using Llama 3 70B, but you can change to mixtral-8x7b-32768)
...
# Embeddings model - Switched to HuggingFace (Free & Local)
...
def chat_node(state: ChatState):
    ...
    system_message = SystemMessage(
        content=(
            "You are a highly capable Agentic Chatbot equipped with specialized tools.\n\n"
            
            "CRITICAL INSTRUCTION FOR DOCUMENTS:\n"
            "You HAVE the ability to read uploaded documents via the `rag_tool`. "
            "NEVER say you cannot read PDFs, cannot access documents, or do not have prior knowledge. "
            "If the user asks about a resume, PDF, document, or specific details like a person's name, "
            "YOU MUST ALWAYS call the `rag_tool` first with the relevant query to search the database.\n\n"
            
            "Tool usage instructions:\n"
            "- Use `rag_tool` immediately for any questions about uploaded PDFs, resumes, or documents.\n"
            "- Use `search_tool` for current events, recent information, or internet searches.\n"
            ...
            "If the `rag_tool` returns 'No relevant information was found', only then should you politely "
            "inform the user that the detail isn't in the uploaded document."
        )
    )
